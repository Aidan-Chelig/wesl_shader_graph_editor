...
pub enum NodeResponse<UserResponse: UserResponseTrait, NodeData: NodeDataTrait> {
    ...
    HoverNode(NodeId),
    SelectNode(NodeId),
    SelectConnectedNode(NodeId),
    ContextNode(NodeId, Pos2),
    ...
    fn draw_graph_editor_inside_zoom(
        ...
        for response in delayed_responses.iter() {
            match response {
                ...
                NodeResponse::HoverNode(_) => {}
                NodeResponse::SelectConnectedNode(_) => {}
                NodeResponse::ContextNode(_, _) => {
                    context_node_requested = true;
                }
        ...
        if mouse.secondary_released() && !cursor_in_finder && !context_node_requested {
            self.node_finder = Some(NodeFinder::new_at(cursor_pos));
        }
    ...
    DataType: DataTypeTrait<UserState>,
{
    ...
    fn show_graph_node(
        ...
        let mut child_ui = ui.new_child(
            UiBuilder::new()
                .max_rect(inner_rect)
                .layout(*ui.layout())
                .id_salt(("node", self.node_id, "inner"))
                .global_scope(true),
        );
            ...
            NodeData: NodeDataTrait,
        {
            ...
            let resp = ui.interact(port_rect, Id::new((node_id, param_id, "port")), sense);
        ...
        if can_flip && Self::flip_button(pan_zoom, ui, outer_rect, self.node_id).clicked() {
        ...
        if can_delete && Self::close_button(pan_zoom, ui, outer_rect, self.node_id).clicked() {
        ...
        if window_response.hovered() {
            responses.push(NodeResponse::HoverNode(self.node_id));
        }
        ...
        let only_hover_response = responses
            .iter()
            .all(|response| matches!(response, NodeResponse::HoverNode(_)));
        if only_hover_response && window_response.clicked_by(PointerButton::Primary) {
            if ui.input(|i| i.modifiers.ctrl) {
                responses.push(NodeResponse::SelectConnectedNode(self.node_id));
            } else {
                responses.push(NodeResponse::SelectNode(self.node_id));
            }
        }

        if window_response.clicked_by(PointerButton::Secondary) {
            if let Some(pos) = ui.ctx().pointer_latest_pos() {
                responses.push(NodeResponse::ContextNode(self.node_id, pos));
            }
        }
    ...
    fn close_button(pan_zoom: &PanZoom, ui: &mut Ui, node_rect: Rect, node_id: NodeId) -> Response {
        // Measurements
        ...
        let resp = ui.interact(rect, Id::new((node_id, "close")), Sense::click());
    ...
    fn flip_button(pan_zoom: &PanZoom, ui: &mut Ui, node_rect: Rect, node_id: NodeId) -> Response {
        // Measurements
        let margin = 8.0 * pan_zoom.zoom;
        let size = 10.0 * pan_zoom.zoom;
        let stroke_width = 2.0;
        ...
        let rect = Rect::from_center_size(position, vec2(size, size));
        let resp = ui.interact(rect, Id::new((node_id, "flip")), Sense::click());
